...
// מפה פשוטה של מילות מפתח -> קטגוריות
...
// מחלץ קטגוריות/ציונים מטקסט חלום בצורה היוריסטית
function categorize(text: string): {
  ...
  for (const cat of DREAM_CATEGORIES) {
    ...
    for (const rgx of regs) {
      ...
      if (matches?.length) score += Math.min(0.4 + matches.length * 0.15, 1); // מצטבר רך
    }
    if (score > 0) scores[cat] = clamp01(score);
  }
...
/**
 * יוצר חלומות בעברית עם פירוש עשיר ומפורט + קטגוריות.
 * אם קיימים כבר חלומות – מדלג (כמו קודם).
 * ניתן להעביר force=true כדי להתעלם ולזרוע בכל מקרה.
 */
async function seedDreams(count: number = 10, force: boolean = false) {
  ...
  const dreamTemplates = [
    {
      ...
      interpretation:
        "המים מסמלים רגשות וזרימה פנימית... המסר הוא ללמוד לסמוך על הזרימה מבלי להילחם בה.",
    },
    {
      ...
      interpretation:
        "נפילה מסמלת לעיתים אובדן ביטחון או פחד מכישלון... דווקא דרך הוויתור תמצא קרקע יציבה חדשה.",
    },
    {
      title: "חלום על תעופה חופשית בשמיים",
      text: "חלמתי שאני עף גבוה מעל הערים, מסתכל על העולם מלמעלה, מרגיש חופש מוחלט.",
      interpretation:
        "עוף בחלום מסמל חופש, העצמה ושחרור ממגבלות... התפתחות וצמיחה פנימית.",
    },
    {
      ...
      interpretation:
        "שיניים נופלות בחלום קשורות לרוב לתחושת חוסר ביטחון או פחד מאיבוד כוח אישי...",
    },
    {
      ...
      interpretation:
        "המרדף מסמל פחדים לא מודעים או נושאים שאתה נמנע מלהתמודד איתם...",
    },
    {
      title: "חלום על ים סוער",
      text: "מצאתי את עצמי בלב ים, הגלים היו עצומים והרוח חזקה. הרגשתי שאני נמשך מטה אבל גם נלחם כדי לצוף.",
      interpretation:
        "ים סוער בחלום הוא סמל קלאסי לעולם רגשי סוער... מראה על כוחות הישרדות ויכולת להתמודד עם קשיים.",
    },
  ];
  ...
  console.log(
    `✅ Inserted ${created.length} enriched Hebrew dreams (with categories)`
  );
  return created;
}
