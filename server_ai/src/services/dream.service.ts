...
function buildFilter(input: {
    ...
    const searchFilter = trimmed.length > 0
        ? {
            $or: [
    ...
    const andFilters = [baseFilter, searchFilter, categoriesFilter].filter(Boolean);
    return andFilters.length > 1
        ? { $and: andFilters }
        : andFilters[0] || baseFilter;
}
export const saveDream = async (userId: string, title: string, userInput: string, aiResponse: string, isShared: boolean = false, categories: DreamCategory[] = [], categoryScores?: Record<string, number>) => {
...
export const createDreamFromInterpretation = async (userId: string, userInput: string, interpretation: string, titleOverride?: string, opts?: {
    isShared?: boolean;
    categories?: DreamCategory[];
    ...
    return saveDream(userId, title, userInput, interpretation, isShared, opts?.categories ?? [], opts?.categoryScores);
};
export const createDreamWithAI = async (userId: string, userInput: string, titleOverride?: string, llmOptions?: LLMOptions & {
    isShared?: boolean;
}) => {
    if (!userId)
        throw new Error("userId is required");
    ...
    const { title, interpretation, categories = [], categoryScores, } = await llm.interpretDream(userInput, llmOptions);
    const finalTitle = (titleOverride && titleOverride.trim()) ||
        (title && title.trim()) ||
        "חלום ללא כותרת";
    const isShared = typeof llmOptions?.isShared === "boolean" ? llmOptions.isShared : false;
    return saveDream(userId, finalTitle, userInput, interpretation, isShared, categories, categoryScores);
};
export const updateDream = async (id: string, data: Partial<{
    title: string;
    userInput: string;
    aiResponse: string;
    isShared: boolean;
...
export const getDreams = async (query: GetDreamsQuery) => {
    const { viewerId, userId, ownerId, search, sortBy, order = "desc", page = 1, limit = 10, categories, } = query;
