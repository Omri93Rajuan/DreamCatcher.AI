...
export default async function authenticate(req: AuthRequest, res: Response, next: NextFunction): Promise<void> {
    try {
        ...
        const user = await User.findById(userId).select("_id role isActive passwordChangedAt");
