...
function normalizeCategories(input: unknown): DreamCategory[] {
    ...
    for (const v of arr) {
        ...
        if (s && !allowedSet.has(s)) {
            console.warn(`⚠️ Category "${s}" not in allowed list:`, Array.from(allowedSet));
        }
        if (s && allowedSet.has(s) && !out.includes(s))
            out.push(s);
    }
    console.log("📊 Normalized categories:", out);
    return out as DreamCategory[];
}
function normalizeCategoryScores(input: unknown): Record<DreamCategory, number> | undefined {
...
export const getAllDreams: RequestHandler = async (req, res): Promise<void> => {
    try {
        ...
        const qCatsRaw = (req.query.categories ?? req.query.category) as string | string[] | undefined;
...
export const createDream: RequestHandler = async (req, res): Promise<void> => {
    try {
        const { userId } = getAuth(req as AuthRequest);
        const { userInput, aiResponse, title, isShared, model, categories, categoryScores, } = req.body ?? {};
        if (!userId) {
        ...
        const saved = aiResponse
            ? await DreamService.createDreamFromInterpretation(userId, userInput, String(aiResponse), title, {
                isShared: !!isShared,
...
export const interpretDream: RequestHandler = async (req, res): Promise<void> => {
    try {
        const { userId } = getAuth(req as AuthRequest);
        if (!userId) {
            res.status(401).json({ success: false, error: "auth_required" });
        ...
        const { text, userInput, prompt, dream_text, isShared, model, titleOverride, } = req.body ?? {};
        ...
        const { title, interpretation, categories = [], categoryScores, } = await llm.interpretDream(rawText, {
            ...(model ? { modelOverride: model } : {}),
        });
        const saved = await DreamService.createDreamFromInterpretation(userId, rawText, interpretation, (titleOverride && String(titleOverride).trim()) || title, {
            isShared: !!isShared,
            categories,
            categoryScores,
        });
...
export const updateDream: RequestHandler = async (req, res): Promise<void> => {
    try {
        ...
        const { title, userInput, aiResponse, isShared, categories, categoryScores, } = req.body ?? {};
        const patch: any = {};
        if (typeof title === "string")
            patch.title = title;
        if (typeof userInput === "string")
            patch.userInput = userInput;
        if (typeof aiResponse === "string")
            patch.aiResponse = aiResponse;
        if (typeof isShared === "boolean")
            patch.isShared = isShared;
...
export const getDreamCategories: RequestHandler = async (_req, res): Promise<void> => {
    try {
...
export const getDreamStats: RequestHandler = async (req, res): Promise<void> => {
    try {
