...
export default function AuthSignup({ onSuccess }: Props) {
  ...
  const [eErr, setEErr] = useState(""),
    [fErr, setFErr] = useState(""),
    [lErr, setLErr] = useState(""),
    [pErr, setPErr] = useState(""),
    [aErr, setAErr] = useState("");
  ...
  const vAvatar = () =>
    setAErr(AVATAR_REQUIRED && !file ? "נא להעלות תמונה." : "");
  ...
  const onFile = (f: File | null) => {
    ...
    if (f.size > 5 * 1024 * 1024) return setAErr("עד 5MB.");
    setFile(f);
  };
  ...
  const register = async () => {
    ...
    try {
      ...
      const s = e?.response?.status,
        m = e?.response?.data?.message || e?.message || "שגיאה.";
      if (s === 409) {
        setStep("email");
  ...
  return (
    <div
      dir="rtl"
      className="w-full max-w-md mx-auto p-6 rounded-2xl glass-card space-y-6 text-slate-900 dark:text-white"
    >
      <div className="text-xs text-slate-500 dark:text-white/70">
        {["email", "details", "avatar", "terms"].indexOf(step) + 1} / 4
      </div>
      {err && (
        <div className="rounded-xl px-3 py-2 text-sm border bg-rose-500/10 text-rose-700 border-rose-400/30 dark:bg-rose-500/15 dark:text-rose-100 dark:border-rose-400/30">
          {err}
        </div>
      )}
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              className={`w-full rounded-xl px-3 py-2 outline-none border bg-white/80 text-slate-900 placeholder:text-slate-400 border-black/10 focus:ring-2 focus:ring-amber-400/40 focus:border-amber-400/60 dark:bg-white/[0.06] dark:text-white dark:placeholder:text-white/40 dark:border-white/10 ${
                eErr && "border-rose-400/60"
              }`}
            />
            {eErr && (
              <p className="mt-1 text-rose-500 dark:text-rose-300 text-xs">
                {eErr}
              </p>
            )}
          </div>
          <button
            disabled={!emailOk}
            className="w-full py-3 rounded-xl font-semibold text-white transition bg-[linear-gradient(135deg,#8b5cf6_0%,#f59e0b_100%)] shadow-[0_8px_24px_-16px_rgba(0,0,0,.12)] dark:shadow-[0_8px_24px_-16px_rgba(0,0,0,.35)] hover:opacity-95 focus:ring-2 focus:ring-amber-300/40 disabled:opacity-50"
          >
            המשך
          </button>
        </form>
      ...
      {step === "avatar" && (
        ...
        >
            ...
            {preview ? (
              <div className="space-y-3">
                <img
                  src={preview}
                  alt=""
                  className="w-40 h-40 object-cover rounded-2xl border border-black/10 dark:border-white/20 mx-auto"
                />
          ...
          {aErr && (
            <p className="text-rose-500 dark:text-rose-300 text-xs">{aErr}</p>
          )}
          <div className="flex gap-3">
      ...
      {step === "terms" && (
        <form
          onSubmit={(e) => {
            e.preventDefault();
            if (!agreed) return setErr("יש לאשר תנאי שימוש.");
            register();
          }}
          className="space-y-6"
        >
          ...
          <div className="flex items-center gap-2 text-sm text-slate-800 dark:text-white/90">
            ...
            {agreed && (
              <span className="text-xs px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-700 dark:text-emerald-200 border border-emerald-400/30">
                ✓ אושר
              </span>
            )}
          </div>
  ...
  return (
    <div>
      ...
      <input
        ...
        placeholder={label}
        className={`w-full rounded-xl px-3 py-2 outline-none border bg-white/80 text-slate-900 placeholder:text-slate-400 border-black/10 focus:ring-2 focus:ring-amber-400/40 focus:border-amber-400/60 dark:bg-white/[0.06] dark:text-white dark:placeholder:text-white/40 dark:border-white/10 ${
          err && "border-rose-400/60"
        }`}
      />
      {err && (
        <p className="mt-1 text-rose-500 dark:text-rose-300 text-xs">{err}</p>
      )}
    </div>
  );
}
function BtnGrad(props: any) {
  return (
    <button
      {...props}
      className={`flex-1 py-2.5 rounded-xl font-semibold text-white transition bg-[linear-gradient(135deg,#8b5cf6_0%,#f59e0b_100%)] shadow-[0_8px_24px_-16px_rgba(0,0,0,.12)] dark:shadow-[0_8px_24px_-16px_rgba(0,0,0,.35)] hover:opacity-95 focus:ring-2 focus:ring-amber-300/40 disabled:opacity-50 disabled:cursor-not-allowed ${
        props.className || ""
      }`}
    />
  );
...
function BtnGhost(props: any) {
  return (
    <button
      type="button"
      {...props}
      className={`flex-1 py-2.5 rounded-xl font-medium transition bg-white/70 text-slate-700 border border-black/10 hover:bg-white dark:bg-white/[0.06] dark:text-white dark:border-white/10 dark:hover:bg-white/[0.1] ${
        props.className || ""
      }`}
    />
  );
