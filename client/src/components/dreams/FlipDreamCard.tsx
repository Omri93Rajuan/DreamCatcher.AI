...
import { Gauge, Heart, Eye, Clock, ArrowUpRight, ArrowDownRight, } from "lucide-react";
...
const fmtLabel = (iso: string) => {
    ...
    return dt.toLocaleDateString("he-IL", iso.length >= 10
        ? { day: "2-digit", month: "2-digit" }
        : { month: "2-digit", year: "2-digit" });
};
export default function FlipDreamCard({ row, windowDaysLabel, hideRank = false, hideChartIfEmpty = true, }: {
    ...
    React.useEffect(() => {
        if (dreamId)
            DreamsApi.recordActivity(dreamId, "view");
    }, [dreamId]);
    const userInput = (dream?.userInput ?? "").trim();
    const interpretation = ((dream as any)?.aiResponse ??
    ...
    return (<div className="
      ...
      " onClick={() => setFlipped((v) => !v)} role="button" aria-label="הפוך כרטיס" tabIndex={0} onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && setFlipped((v) => !v)} style={{ perspective: 1000 }} dir="rtl">
      <motion.div className="w-full h-full" animate={{ rotateY: flipped ? 180 : 0 }} transition={{ duration: 0.55, ease: [0.2, 0.8, 0.2, 1] }} style={{ transformStyle: "preserve-3d" }}>
        
        <div className="absolute inset-0 p-4" style={{ backfaceVisibility: "hidden" }}>
          {!hideRank && (<div className="absolute top-3 right-3 z-10 text-xs font-bold bg-amber-400 text-slate-900 rounded-full px-2 py-1">
              #{row.rank}
            </div>)}
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-3 text-sm">
            ...
            {row.percentChange !== null && (<span className={[
                "inline-flex items-center gap-1",
                (row.percentChange ?? 0) >= 0
                    ? "text-emerald-600 dark:text-emerald-400"
                    : "text-rose-600 dark:text-rose-400",
            ].join(" ")} title="שינוי מול התקופה הקודמת">
                {(row.percentChange ?? 0) >= 0 ? (<ArrowUpRight className="w-4 h-4"/>) : (<ArrowDownRight className="w-4 h-4"/>)}
          ...
          <div className="h-28 mt-3 rounded-md text-sky-600 dark:text-sky-300">
            {series.length > 0 ? (<ResponsiveContainer width="100%" height="100%">
                <AreaChart data={series} margin={{ left: 0, right: 0, top: 6, bottom: 0 }}>
                  ...
                  <YAxis hide/>
                  <Tooltip formatter={(v: any) => [nf.format(Number(v)), "ציון"]} labelFormatter={(label) => typeof label === "string" ? fmtLabel(label) : ""}/>
                  <Area type="monotone" dataKey="score" stroke="currentColor" strokeWidth={2} fill={`url(#grad-${dreamId})`} dot={false} isAnimationActive={false}/>
                </AreaChart>
              </ResponsiveContainer>) : hideChartIfEmpty ? null : (<div className="h-full w-full rounded-md bg-black/5 dark:bg-white/5"/>)}
          </div>
          ...
          <div className="grid grid-rows-[1fr_auto] h-[calc(100%-2rem)]">
            <div className="
              ...
              {userInput ? (<div>
                ...
                </div>) : (<div className="text-slate-600 dark:text-white/60">
                  טוען חלום…
                </div>)}
