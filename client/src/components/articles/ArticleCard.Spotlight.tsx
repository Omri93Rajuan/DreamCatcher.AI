  ...
  return (
    ...
    >
        ...
        {a.tags?.length ? (
          <ul className="mt-3 flex flex-wrap gap-1.5">
            {a.tags.slice(0, 3).map((t) => (
              <li
                key={t}
                className="text-[11px] px-2 py-0.5 rounded-full bg-slate-900/5 dark:bg-white/10 text-slate-600 dark:text-white/70"
              >
                #{t}
              </li>
            ))}
      ...
      <div className="relative">
        <div className="aspect-[4/3] md:aspect-auto md:h-full overflow-hidden">
          {a.coverUrl ? (
            <img
              src={a.coverUrl}
              alt=""
              className="w-full h-full object-cover transition duration-500 md:scale-100 hover:scale-[1.03]"
              loading="lazy"
            />
          ) : (
        ...
        {/* תגיות על התמונה (עדין, לא יזלוג) */}
        {a.tags?.length ? (
          <div className="absolute bottom-3 left-3 flex gap-1.5">
            {a.tags.slice(0, 2).map((t) => (
              <span
                key={t}
                className="text-[10px] px-2 py-0.5 rounded-full bg-black/45 text-white/95 backdrop-blur-sm border border-white/20"
              >
                #{t}
              </span>
            ))}
