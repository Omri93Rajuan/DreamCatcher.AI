...
export default function ArticlesGrid({ items, onOpen, }: {
    items: Article[];
    onOpen: (a: Article) => void;
}) {
    return (<section aria-label="רשימת מאמרים" className="space-y-6">
      ...
      <div className="grid gap-6 sm:grid-cols-1 lg:grid-cols-2">
        {items.map((a) => (<ArticleCardSplit key={a.id} a={a} onOpen={() => onOpen(a)}/>))}
      </div>
    </section>);
}
