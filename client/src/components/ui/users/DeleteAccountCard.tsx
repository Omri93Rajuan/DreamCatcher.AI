...
export default function DeleteAccountCard({ userId }: Props) {
  ...
  const onDelete = async () => {
    ...
    setLoading(true);
    try {
      await AuthApi.deleteAccount(id); // DELETE /users/:id
      ...
      // Fallback: הודעה לעמוד הבא (למקרה שהטוסט לא הספיק להופיע)
      try {
      ...
      // נבצע logout ואז ניווט קטן אחרי שהטוסט “חי” קצת
      setTimeout(() => {
        logout?.();
        window.location.assign("/login");
      }, 1400);

      // לא נוגעים בסטייט אחרי הניווט
    } catch {
      setLoading(false);
