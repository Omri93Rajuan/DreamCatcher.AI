...
export default function GoogleCallbackPage() {
  ...
  const [message, setMessage] = useState("מתחברים לחשבון Google...");
  ...
  useEffect(() => {
    if (initialStatus === "error") {
      setStatus("error");
      setMessage(initialMessage || "לא הצלחנו לאמת את החשבון. נסו שוב או הירשמו.");
      return;
    }
    let cancelled = false;
    (async () => {
      try {
        ...
        setStatus("error");
        setMessage("לא הצלחנו לאמת את החשבון. נסו להתחבר שוב או הירשמו.");
      } catch {
        if (!cancelled) {
          setStatus("error");
          setMessage("שגיאה בתהליך ההתחברות. נסו שוב או הירשמו.");
        }
      }
    })();
    return () => {
  ...
  }, [initialStatus, initialMessage, navigate, next, setUser]);
